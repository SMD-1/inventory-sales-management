...
const Product = () => {
  ...
  const fetchStats = async () => {
    try {
      const res = await get("/api/products/product-stats");
      const metrics = res.data || res;
      if (metrics) {
        setOverallInventory([
          {
            name: "Categories",
            items: [{ name: "Total", value: metrics.categories || 0 }]
          },
          {
            name: "Total Products",
            items: [
              { name: "Total", value: metrics.totalProducts || 0 },
              { name: "Stock Value", value: `₹${metrics.totalStockValue || 0}` }
            ]
          },
          {
            name: "Top Sellings",
          ...
          },
          {
            name: "Low Stocks",
            items: [
              { name: "Low Stock", value: metrics.lowStockCount || 0 },
              { name: "Out of Stock", value: metrics.outOfStockCount || 0 }
            ]
          }
        ]);
      }
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/products/bulk`,
        {
          method: "POST",
          headers: {
      ...
      fetchProducts(1); // Refresh list to show newly added on first page
    } catch (error) {
  ...
  const processedProducts = products.map((product) => {
    ...
    return { ...product, availability, availabilityClass };
  });
              ...
              <div className="items">
                {item.items.map((subItem, subIndex) => (
                  <div className="item" key={subIndex}>
                    <p className="amount">{subItem.value}</p>
                    <p className="name">{subItem.name}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <tbody>
              ...
              {processedProducts.length === 0 && (
                <tr>
                  ...
                  >
                    {searchQuery ? "No matching products found." : "No products found. Add Product."}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {isCSVModalOpen && (
        <div className="modal-overlay" onClick={() => setIsCSVModalOpen(false)}>
          ...
          >
            ...
            {csvFile && (
              <div className="csv-file" style={{ marginTop: "20px" }}>
                ...
                <button className="delete-btn" onClick={() => setCsvFile(null)}>
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
